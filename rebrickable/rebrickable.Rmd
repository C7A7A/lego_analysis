...
p <- ggplot(sets_stats_long, aes(x = year, y = value, color = variable)) +
  ...
  labs(title = "Yearly Sets Statistics",
       x = "Year",
       y = "Values") +
  theme_minimal() +
...
# knitr::kable(head(inventory_minifigures))
...
forecast_model_sets_parts <- auto.arima(time_series_data_sw_parts)
forecast_parts_sw <- forecast(forecast_model_sets_parts, h=10)
